...
openxlsx::write.xlsx(DATASET, file = "DataRecords_OuputData_pox.xlsx")
write.csv(x = DATASET, file = "DataRecords_OutputData.csv")


#### EXPORT THE REGRESSION PLOTS ####
pdf(file = "output_adjusted_kinetics/regression_plots.pdf",
    width = 4,
    height = 4)
for (i in 1:length(regplot)) {
  print(regplot[[i]])
}
dev.off()
