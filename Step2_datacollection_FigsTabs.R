...
# For Figures, the images are provided in the 'publication_figures' directory.
...
for (i in 1:length(dd$scatterplot)) { # for each digitalized figure
  ...
  df <- rbind(df, data.frame(Kinetic_key = dd$scatterplot[[i]]$id, # 'id' (Kinetics_key),
                             time_min = dd$scatterplot[[i]]$x, # x variable (time in minutes),
...
save.image(file = "Kinetics_data_pox.RData")
